    ...
    try {
        ...
        async function updateProductPrice(productId, price, weight, category) {
            try {
                ...
                const { data: product  } = await fetch(`https://api.bigcommerce.com/stores/${process.env.STORE_HASH}/v3/catalog/products/${productId}`, requestOptions).then((response)=>response.json()).then((result)=>{
                    return result;
                ...
                return await fetch(`https://api.bigcommerce.com/stores/${process.env.STORE_HASH}/v3/catalog/products/${productId}`, requestOptions2).then((response)=>response.json()).catch((error)=>console.log("error", error));
            } catch (error) {
                return res.status(500).json({
                    error: `Error updating product price: ${error}`
                });
            }
        }
        ...
        async function updateAllProductPrices() {
            try {
                ...
                const { data: products  } = await fetch(`https://api.bigcommerce.com/stores/${process.env.STORE_HASH}/v3/catalog/products?limit=300`, requestOptions).then((response)=>response.json()).then((result)=>{
                    return result;
                }).catch((error)=>console.log("error", error));
                // Get Category Spot Prices
                const spotPrice = await fetch(`https://api.metalpriceapi.com/v1/latest?api_key=8ff698fd072b23143d22d462f49580bf&base=GBP`).then((response)=>response.json()).then((data)=>{
                    return data.rates;
                }).catch((error)=>{
                    console.error(error);
                });
                    ...
                    for(const metal in metalPrices){
                        if (name.toLowerCase().includes(metal.toLowerCase())) {
                            ...
                            if (product && productId && spotPrice[cat]) {
                                ...
                                await updateProductPrice(productId, 1 / spotPrice[cat] / 31.1035, weight, metal);
                            }
                        }
                    }
                }
                ...
                return res.status(500).json({
                    error: `Error updating category prices: ${error}`
                });
            }
        }
